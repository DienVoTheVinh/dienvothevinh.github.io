  ...
  function renderManage() {
    ...
    if (!klass) {
      ...
      main = '<div class="portal-manage-main"><section class="card portal-class-overview"><div><span class="portal-chip">Khối ' + esc(klass.grade || '—') + '</span><h3>' + esc(klass.name) + '</h3><p>' + esc(klass.note || ('Niên khóa ' + klass.school_year)) + '</p></div><div class="portal-row-actions"><button class="btn btn-secondary btn-sm" onclick="VMPortalClassroom.openClassForm(\'' + klass.id + '\')">Sửa lớp</button><button class="btn btn-ghost btn-sm portal-danger" onclick="VMPortalClassroom.deleteClass(\'' + klass.id + '\')">Xóa lớp</button></div></section>' +
        ...
        '<section class="portal-manage-section"><div class="portal-manage-head"><div><span>3</span><div><h3>Đề thi</h3><p>Xưởng riêng cho đề THPTQG; biểu mẫu nhanh cũ vẫn được giữ lại.</p></div></div><div class="portal-row-actions"><a class="btn btn-primary btn-sm" href="quan-tri-de?portal=' + encodeURIComponent(vmPortalState.portal.slug) + '">🛠 Mở xưởng soạn đề</a><button class="btn btn-secondary btn-sm" onclick="VMPortalClassroom.openExamForm()">+ Tạo nhanh</button></div></div><div class="portal-manage-list">' + examRows + '</div></section></div>';
    }
