...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Phan quyen: GV/TG chi Admin; HS+PH cho Admin & Giao vien
    if (["gv", "tg"].includes(type)) {
      if (prof.role !== "admin") return jsonRes({ error: "Chi Quan tri duoc tao tai khoan GV/TG" }, 403);
    } else {
      if (!["admin", "teacher"].includes(prof.role)) return jsonRes({ error: "Chi Quan tri / Giao vien duoc tao tai khoan" }, 403);
    }
    ...
    if (type === "hs_ph") {
      ...
      const phEmail = u + "@" + DOMAIN.ph; // truoc @ la ten, sau @ la chuc vu
