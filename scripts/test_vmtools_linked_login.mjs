...
test('invalid inputs are rejected before any account lookup',async()=>{for(const bad of [{email:null,password:'x'},{email:'bad whitespace',password:'x'},{email:'a@b.com',password:''},{email:'a@b.com',password:'x'.repeat(129)}])await assert.rejects(linkedPasswordLogin(bad,{account:()=>assert.fail()}),/không đúng/);});

test('student username and short suffix resolve the same linked account without changing role',async()=>{for(const email of ['studentone','studentone@hs','studentone@hs.vinhmath.com']){const services={account:async v=>{assert.equal(v,'studentone@hs.vinhmath.com');return {auth_user_id:'student-id',status:'active'};},identity:async()=>({email:'studentone@hs.vinhmath.com'}),signIn:async()=>({data:{user:{id:'student-id'},session:{access_token:'student-token'}}})};assert.equal(await linkedPasswordLogin({email,password:'password-fixture'},services),'student-token');}});
