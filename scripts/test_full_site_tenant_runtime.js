...
expect(/item\.featureKey === 'home'[\s\S]{0,180}vmTenantHomePath\(fullSiteTenant\)/.test(menu), 'The tenant home menu item must point to its own landing page');
...
expect(shared.includes("'trang-chu':'home','uyenmath':'home'"), 'Both the normal and branded home routes must be guarded');
expect(shared.includes('function vmTenantFirstShownPath') && shared.includes("['lessons', 'lop-hoc']"), 'The route guard needs a role-appropriate shown-route fallback');
