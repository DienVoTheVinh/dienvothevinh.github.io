...
expect(worker.includes("vinhmath-shell-v43"), 'Service worker cache version must publish the restored VinhMath brand palette');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '43'"), 'Open apps must not enter a refresh loop on shell v43');
expect(worker.includes("target.searchParams.set('vm_refresh', '43'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=43'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
