...
expect(worker.includes("vinhmath-shell-v39"), 'Service worker cache version must publish the auth recovery hotfix');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '39'"), 'Open apps must not enter a refresh loop on shell v39');
expect(worker.includes("target.searchParams.set('vm_refresh', '39'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=39'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
