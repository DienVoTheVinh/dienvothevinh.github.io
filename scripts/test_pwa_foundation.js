...
expect(worker.includes("vinhmath-shell-v55"), 'Service worker cache version must publish the fixed hidden bank notice');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '50'"), 'Open apps must not enter a refresh loop on shell v55');
expect(worker.includes("target.searchParams.set('vm_refresh', '50'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=51'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
