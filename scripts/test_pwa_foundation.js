...
expect(worker.includes("vinhmath-shell-v41"), 'Service worker cache version must publish the lazy VMTool workspace');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '41'"), 'Open apps must not enter a refresh loop on shell v41');
expect(worker.includes("target.searchParams.set('vm_refresh', '41'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=41'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
