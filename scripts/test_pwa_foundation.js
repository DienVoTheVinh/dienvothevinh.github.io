...
expect(worker.includes("vinhmath-shell-v36"), 'Service worker cache version must publish the security and performance release');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '36'"), 'Open apps must not enter a refresh loop on shell v36');
expect(worker.includes("target.searchParams.set('vm_refresh', '36'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=36'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
...
for (const file of htmlFiles) {
  ...
  expect(!/css\/vinhmath\.css\?v=(?!8\.3)/.test(html), `${file}: stale shared CSS version`);
