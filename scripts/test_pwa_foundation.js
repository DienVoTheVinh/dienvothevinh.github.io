...
expect(worker.includes("vinhmath-shell-v69"), 'Service worker cache version must publish snapshot-free navigation');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '69'"), 'Open apps must not enter a refresh loop on shell v69');
expect(worker.includes("target.searchParams.set('vm_refresh', '69'"), 'Open apps must reload once after the new shell activates');
