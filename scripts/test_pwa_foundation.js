...
expect(worker.includes("vinhmath-shell-v42"), 'Service worker cache version must publish the unified student journey');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '42'"), 'Open apps must not enter a refresh loop on shell v42');
expect(worker.includes("target.searchParams.set('vm_refresh', '42'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=42'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
