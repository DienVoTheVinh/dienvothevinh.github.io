...
expect(worker.includes("vinhmath-shell-v54"), 'Service worker cache version must publish the empty-bank guidance and linked bank preview');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '49'"), 'Open apps must not enter a refresh loop on shell v54');
expect(worker.includes("target.searchParams.set('vm_refresh', '49'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=50'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
