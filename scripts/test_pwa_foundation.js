...
expect(worker.includes("vinhmath-shell-v68"), 'Service worker cache version must publish the stable navigation shell');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '68'"), 'Open apps must not enter a refresh loop on shell v68');
expect(worker.includes("target.searchParams.set('vm_refresh', '68'"), 'Open apps must reload once after the new shell activates');
...
for (const file of htmlFiles) {
  ...
  expect(!/css\/vinhmath\.css\?v=(?!8\.8)/.test(html), `${file}: stale shared CSS version`);
