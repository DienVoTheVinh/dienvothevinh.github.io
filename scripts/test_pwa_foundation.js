...
expect(worker.includes("vinhmath-shell-v34"), 'Service worker cache version must publish the balanced student home release');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '34'"), 'Open apps must not enter a refresh loop on shell v34');
expect(worker.includes("target.searchParams.set('vm_refresh', '34'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=34'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
