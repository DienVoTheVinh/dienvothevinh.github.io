...
expect(worker.includes("vinhmath-shell-v35"), 'Service worker cache version must publish the wide material reader release');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '35'"), 'Open apps must not enter a refresh loop on shell v35');
expect(worker.includes("target.searchParams.set('vm_refresh', '35'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=35'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
