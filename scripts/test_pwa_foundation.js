...
expect(worker.includes("vinhmath-shell-v70"), 'Service worker cache version must publish schedule-scoped attendance');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '70'"), 'Open apps must not enter a refresh loop on shell v70');
expect(worker.includes("target.searchParams.set('vm_refresh', '70'"), 'Open apps must reload once after the new shell activates');
...
for (const file of htmlFiles) {
  ...
  expect(!/css\/vinhmath\.css\?v=(?!8\.9)/.test(html), `${file}: stale shared CSS version`);
