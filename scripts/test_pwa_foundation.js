...
expect(worker.includes("vinhmath-shell-v44"), 'Service worker cache version must publish the restored VinhMath logo and brand palette');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=44'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
