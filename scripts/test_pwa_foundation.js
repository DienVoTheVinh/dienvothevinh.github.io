...
expect(worker.includes("vinhmath-shell-v59"), 'Service worker cache version must publish the tenant builder shell');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '59'"), 'Open apps must not enter a refresh loop on shell v59');
expect(worker.includes("target.searchParams.set('vm_refresh', '59'"), 'Open apps must reload once after the new shell activates');
