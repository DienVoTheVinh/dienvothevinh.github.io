...
expect(worker.includes("vinhmath-shell-v63"), 'Service worker cache version must publish the top-layer student-result media fix');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '63'"), 'Open apps must not enter a refresh loop on shell v63');
expect(worker.includes("target.searchParams.set('vm_refresh', '63'"), 'Open apps must reload once after the new shell activates');
