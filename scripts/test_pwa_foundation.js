...
expect(worker.includes("vinhmath-shell-v26"), 'Service worker cache version must publish the complete content studio release');
expect(worker.includes("VM_PREVIOUS_POPUP_CACHE = 'vinhmath-shell-v25'"), 'Service worker must detect the previous application shell');
expect(worker.includes("target.searchParams.get('vm_refresh') === '26'"), 'Open apps must not enter a refresh loop on shell v26');
expect(worker.includes("target.searchParams.set('vm_refresh', '26'"), 'Open apps must reload once after the new shell activates');
