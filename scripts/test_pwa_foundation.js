...
expect(worker.includes("vinhmath-shell-v45"), 'Service worker cache version must publish the compact student home and fullscreen results viewer');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '44'"), 'Open apps must not enter a refresh loop on shell v44');
expect(worker.includes("target.searchParams.set('vm_refresh', '44'"), 'Open apps must reload once after the new shell activates');
...
expect(sharedJs.includes("navigator.serviceWorker.register('/sw.js?v=45'") && sharedJs.includes("updateViaCache: 'none'"), 'Shared JS must bypass stale HTTP cache when updating the service worker');
