...
expect(worker.includes("vinhmath-shell-v64"), 'Service worker cache version must publish the legacy-ID and TeX-render fixes');
...
expect(worker.includes("target.searchParams.get('vm_refresh') === '64'"), 'Open apps must not enter a refresh loop on shell v64');
expect(worker.includes("target.searchParams.set('vm_refresh', '64'"), 'Open apps must reload once after the new shell activates');
...
for (const file of htmlFiles) {
  ...
  expect(!/css\/vinhmath\.css\?v=(?!8\.5)/.test(html), `${file}: stale shared CSS version`);
