...
function fixture(role='admin'){
 const calls=[],state={role,session:true},tables={profiles:[{id:'admin',role},{id:'teacher',role:'teacher'},{id:'student',role:'student'}],vmtools_accounts:[{id:'owner',auth_user_id:'admin',role:'owner',status:'active'},{id:'license',auth_user_id:'teacher',role:'user',status:'active',features:['ink'],name:'GV',max_devices:1,offline_days:2,web_enabled:true},{id:'student-license',auth_user_id:'student',role:'user',status:'active'}],vmtools_devices:[{key_hash:'deviceA',account_id:'license',status:'pending'}]};let handler;
 const db={auth:{getUser:async()=>({data:{user:{id:'admin'}},error:null})},from(table){let filters=[],one=false;const q={select(){return q;},eq(k,v){filters.push(r=>r[k]===v);return q;},in(k,v){filters.push(r=>v.includes(r[k]));return q;},single(){one=true;return q;},maybeSingle(){one=true;return q;},order(){return q;},limit(){return q;},update(p){calls.push({table,p});return q;},upsert(p){calls.push({table,p});return q;},then(resolve){const rows=(tables[table]||[]).filter(r=>filters.every(f=>f(r)));return Promise.resolve({data:one?rows[0]||null:rows,error:null}).then(resolve);}};return q;},rpc:async(name,args)=>{calls.push({name,args});return {data:name==='vmtools_auth_session'?state.session:'2027-01-01',error:null};}};
...
(async()=>{
 ...
 const student=fixture();assert.equal((await student.request({action:'renew',service:'vmtools',userId:'student',plan:'monthly',units:1,amount:0,note:'Explicit student grant',requestId:'grant',confirm:true})).status,200);
 const studentClass=fixture();assert.equal((await studentClass.request({action:'renew',service:'classroom',userId:'student',plan:'monthly',units:1,amount:0,note:'',confirm:true})).status,400);assert.equal(studentClass.calls.filter(x=>x.name==='teacher_service_renew').length,0);
 const device=fixture();assert.equal((await device.request({action:'device',service:'vmtools',userId:'student',key:'deviceA',status:'active'})).status,400);assert.equal(device.calls.filter(x=>x.name==='vmtools_device_status').length,0);
 const review=fixture();assert.equal((await review.request({action:'device',service:'vmtools',userId:'teacher',key:'deviceA',status:'active'})).status,200);assert.equal(review.calls.filter(x=>x.name==='vmtools_device_status').length,1);
