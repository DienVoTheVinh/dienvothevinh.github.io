...
expect(achievementsPage.includes('realm-map-layout') && achievementsPage.includes('id="realmDetail"') && achievementsPage.includes('css/rank-map.css?v=4') && achievementsPage.includes('js/student-achievement-map.js?v=5'), 'Trang cảnh giới phải nạp bố cục bản đồ mới và tài nguyên đã đổi phiên bản');
expect(rankSystem.includes('REALM_MARKS') && rankSystem.includes('majorMark:majorMark') && rankSystem.includes("mark:'constellation'") && rankSystem.includes("mark:'prism'"), 'Mỗi cảnh giới phải dùng huy hiệu SVG riêng, gồm Kỳ Tài và Học Thần khác biệt');
expect(achievements.includes('realm-map-campus') && achievements.includes('realm-map-library') && achievements.includes('realm-map-zones') && achievements.includes('realm-landmark-copy'), 'Bản đồ phải có địa hình học đường chi tiết và nhãn địa danh bố trí riêng');
...
expect(practicePage.includes('.practice-shell{display:grid;grid-template-columns:minmax(0,1fr)') && practicePage.includes('.practice-shell.has-class-filter{grid-template-columns:220px minmax(0,1fr)}'), 'Ẩn bộ lọc lớp không được làm danh sách bài tập co còn 220px');
expect(practicePage.includes('function vmResetExamWorkspace()') && practicePage.includes("url.searchParams.delete('exam_id')"), 'Thoát đề phải dọn sạch trạng thái toàn màn hình và tham số tự mở đề');
