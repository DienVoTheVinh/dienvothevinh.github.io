...
expect(shared.includes('/@hs\\.[a-z0-9]+(?:-[a-z0-9]+)*$/') && login.includes('vmDichDangNhap'), 'Partner login suffix routing is missing');
...
expect(portalAdmin.includes("type:'portal_hs'") && portalAdmin.includes('exam_portal_exams'), 'Portal management workflow is incomplete');
expect(accountFunction.includes('prof.role !== "admin"') && accountFunction.includes('member_role: "student", portal_only: true'), 'Portal account creation must be admin-only and portal-only');
...
expect(!/service_role|SUPABASE_SERVICE_ROLE_KEY/i.test([menu, shared, portal, portalAdmin, exam, migration].join('\n')), 'Privileged credentials must not appear in browser or migration files');
