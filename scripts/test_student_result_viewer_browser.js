...
(async () => {
  ...
  try {
    ...
    await page.setContent(`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>:root{--font-sans:Arial}${css}</style></head><body><button id="origin">Mở ảnh</button></body></html>`);
    ...
    let state = await page.evaluate(() => ({open:document.getElementById('vmResultMediaViewer').classList.contains('open'),counter:document.querySelector('[data-media-counter]').textContent,thumbs:document.querySelectorAll('[data-media-index]').length,closeVisible:document.querySelector('[data-media-action="close"]').getBoundingClientRect().width>0,overflow:document.documentElement.scrollWidth>innerWidth+1}));
    if (!state.open || state.counter !== 'Ảnh 1/3' || state.thumbs !== 3 || !state.closeVisible || state.overflow) throw new Error(`Initial gallery is wrong: ${JSON.stringify(state)}`);
    ...
    state = await page.evaluate(() => ({open:document.getElementById('vmResultMediaViewer').classList.contains('open'),focus:document.activeElement&&document.activeElement.id,rejected:VMStudentResultUI.openMedia([{url:'javascript:alert(1)',fallbackUrl:'javascript:alert(2)',name:'Không an toàn'}],0)}));
    if (state.open || state.focus !== 'origin' || state.rejected !== false) throw new Error(`Gallery close, focus restore or URL allowlist failed: ${JSON.stringify(state)}`);
    console.log('PASS shared result gallery navigation, swipe, mobile layout and safe close');
