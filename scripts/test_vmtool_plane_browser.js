...
(async () => {
  ...
  try {
    ...
    await page.addInitScript(() => { try { localStorage.setItem('vm-theme', 'dark'); } catch (_error) {} });
    ...
    result=await page.evaluate(()=>{const card=document.getElementById('planeCanvasCard'),rect=card.getBoundingClientRect(),exit=card.querySelector('.vmtool-fullscreen-exit'),exitRect=exit.getBoundingClientRect();return{left:Math.round(rect.left),top:Math.round(rect.top),width:Math.round(rect.width),height:Math.round(rect.height),innerWidth,innerHeight,label:document.getElementById('planeFullscreen').textContent,exitVisible:getComputedStyle(exit).display!=='none',exitLeft:Math.round(exitRect.left),exitTop:Math.round(exitRect.top)};});
    if(result.left!==0||result.top!==0||result.width!==result.innerWidth||result.height!==result.innerHeight||!result.label.includes('Thu nhỏ')||!result.exitVisible||result.exitLeft<0||result.exitTop<0)throw new Error(`Plane fullscreen overlay failed: ${JSON.stringify(result)}`);
    await page.click('#planeCanvasCard .vmtool-fullscreen-exit');
    await page.waitForFunction(()=>!document.getElementById('planeCanvasCard').classList.contains('vmtool-pseudo-fullscreen'));
