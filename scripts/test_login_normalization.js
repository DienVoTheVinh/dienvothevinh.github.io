...
expect(fs.readFileSync('dang-nhap.html', 'utf8').includes('js/vinhmath.js?v=9.8'), 'Login page cache version must be bumped');

console.log('PASS resilient student login normalization and safe password retry');

expect(context.vmEmailDangNhap('Teacher.Name@gmail.com') === 'teacher.name@gmail.com', 'Real teacher Gmail must remain intact');
expect(context.vmEmailDangNhap('Teacher@example.app') === 'teacher@example.app', 'A real .app address must remain intact');
